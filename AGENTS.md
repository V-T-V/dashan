...
- **shared/ 12 模块**（见下表）——被三端复用
- **测试 14 个文件 / 245 个用例**（见下表）
...
**共计 2777 行共享逻辑**，零运行时依赖（全 devDeps）。
...
`npm test`（node --test + tsx），共 **245 用例 / 14 文件**，全绿。
...
npm test             # 245 个测试（node --test + tsx）
...
- **shared/ 是三端共享逻辑的唯一源**——夸赞/账本/LLM/难度/历史/卡片/persistence 逻辑放 shared，不要在 src/server/cli 里重复实现。
- 前端 src 的 DOM 层无自动化测试，shared + server 有测试（245 用例）——**改 shared 时务必跑 `npm test` + `type-check` + `lint`**。
...
- `history.ts` 与 `card.ts` 都从 `LedgerEntry[]` 派生，保证「时间线回看」与「分享卡片」讲同一个故事。
