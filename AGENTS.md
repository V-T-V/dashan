...
**功能完整，三种运行形态都成型，覆盖率高。** 1833 测试全过。有已构建的 dist/。

**R13 深度推进已完成**（1662→1833，+171 测试，9 个新功能模块）：
- D1 **语气画像分析器** `toneProfile.ts`——Shannon 熵多样性、主导语气、推荐补足冷门语气
- D2 **抉择模式分析器** `choicePattern.ts`——快/中/慢分布、果断/深思/犹豫/混合型分类、会话节奏
- D3 **道德困境难度评分器** `dilemmaScorer.ts`——四维评分（选项复杂度/道德冲突/模糊性/后果严重度）、张力识别
- D4 **善恶簿叙事生成器** `narrative.ts`——成长叙事、里程碑提取（首条/十全/半百/百倍/连续语气）
- D5 **每日挑战生成器** `dailyChallenge.ts`——确定性种子生成每日 3 题套餐、完成度评级
- D6 **境界进阶预测器** `titleForecast.ts`——当前/下一境界、进度百分比、全级表
- D7 **结局概率预测器** `endingForecast.ts`——三结局概率分布、走向趋势（上升/下降/稳定）
- D8 **玩家画像综合评分** `playerProfile.ts`——四维修为分、画像标签、双人对比
- D9 **抉择时间热力图** `activityHeatmap.ts`——24 小时/7 天分布、晨夜午后均衡作息标签
- D10 综合集成测试 + 文档同步
...
- **shared/ 23 模块**（见下表）——被三端复用，含 round 8 新增的 schools / music / quotes / daily 四大哲学内容模块，deep 轮新增的 dominantTone / endingNarrative（隐藏结局）/ practiceStage（修行阶段）/ parseEnvText（env 解析加固），R5 轮新增的 achievements（修行成就徽章系统），R10 轮新增的第二隐藏结局「执剑尊者」（满级+主导江湖，与既有「辩经尊者」满级+主导学术对称）与 hiddenEndingHint 进度提示引擎
- **测试 51 个文件 / 1598 个用例**（见下表）
...
| `ledgerCore.ts`    | 305  | 善恶簿纯逻辑：记录/8 级称号阶梯/印章/语气统计/结局推导/dominantTone/endingNarrative（含双隐藏结局：辩经尊者=满级+学术 / 执剑尊者=满级+江湖）                             | ✅ 稳定 |
...
| `i18n.ts`          | 397  | 多语言骨架（zh-CN/en-US）+ 英文 SYSTEM_PROMPT + 翻转论证 5 法 i18n key + 导出 STRINGS/TONE_LABELS/CATEGORY_LABELS/DIFFICULTY_LABELS 字典                                 | ✅ 稳定 |
| `customDilemma.ts` | 235  | 自定义困境创建器（输入情境+选项→确定性生成翻转夸赞）                                                                                                                     | ✅ 新增 |
| `stats.ts`         | 453  | 统计面板数据（选项/语气/题材/时长/称号/结局 图表数据）+ practiceStage 修行阶段分类器（5 阶段正交叙事轴）+ hiddenEndingHint 隐藏结局进度提示（双路径：辩经尊者/执剑尊者） | ✅ 稳定 |
| `export.ts`        | 390  | 导出（JSON / Markdown / HTML 三格式，自包含卡片）+ 导出 ENDING_NAME 常量                                                                                                 | ✅ 稳定 |
...
| `i18n-deep.test.ts`                    | 64     | STRINGS 字典完备性(两语言 key 一致/中英必不同)/t() 回退链(非法 locale/缺省/纯函数)/标签映射完备(6 语气 8 题材 3 难度)/titleLabel 边界(负·超大 index)/englishTitles 拷贝隔离/FLIP_ARGUMENTS 5 法 id 顺序/SYSTEM_PROMPT_EN 同构/EN_SAMPLE_SCRIPT 结构/detectLocale 探测鲁棒（深度）                 |
| `export-deep.test.ts`                  | 57     | JSON 往返无损(meta 透传/summary 与 ledgerCore 一致/深拷贝隔离)/parse 非法全返 null/跨格式一致性(同 total/称号/结局/语气)/MD 结构(6 行表/转义 \\                                                                                                                                                   | )/HTML 结构(CSS 变量/5 卡/escapeHtml 全字段)/确定性/纯函数/time 透传/exportLedger 分派+抛错（深度） |
| `theme-deep.test.ts`                   | 56     | THEME_VARS 三主题变量名严格一致/合法 CSS 颜色/ink·gold 两两不同/text 明暗反转/themeCSSText 精确格式(缩进 2 空格)/allThemesCSS 4 块/toThemeId 全类型鲁棒(null·数字·对象)/isThemeId 守卫/localStorage 抛错被吞/往返/setTheme 返回恒合法/recommendedTheme 永不 ancient（深度）                       |
| `custom-dilemma-deep.test.ts`          | 60     | validate 全类型鲁棒(数组·null·数字)/choices 非字符串逐个报错/去重 trim 语义/hashString 顺序敏感/generatePraiseForChoice 模板替换/同 seed 覆盖 5 法 6 语气/固定 seed flipId·tone 仅依赖 index/createCustomDilemma id 严格 A-D/确定性/纯函数/flipDiversity 边界（深度）                             |
| `quotes-deep.test.ts`                  | 45     | 数据完整(categories·tones·difficulties 合法/text 全局唯一)/scoreQuote 精确(三轴=6/题材=3/语气=2/难度缺省+1)/recommendQuotes 边界(limit 0·负·超大)/同分 idx 升序/命中题材全优先/quoteOfTheMoment 永不 undefined/quoteBySeed 取模回绕·负数/renderQuote 精确格式/stats 手动重算（深度）              |
| `music-deep.test.ts`                   | 44     | 数据完整(categories·tones·durationSec 正/id 唯一/覆盖全 8 题材 6 语气)/scoreTrack 精确(三轴+mood 重叠+0.5=6.5)/无难度字段+1/recommendMusic 边界(0·负·超大)/同分 idx 升序/命中优先/findTrack 边界/recommendMusicPack alternatives=2/buildReason 各轴/stats 手动重算/byTag 去重排序（深度）         |
| `ending-hidden-rogue-deep.test.ts`     | 21     | 新隐藏结局「执剑尊者」(满级+主导江湖)触发精确/未满级不触发/戏谑虽破坏阵营但不触发/辩经尊者不回归/平局决胜(学术声明在前)/count 参数独立/两隐藏结局互斥/type 恒 5 种合法值/与基础 endingType 对照(江湖 baseType=灭世被隐藏覆盖)                                                                     |
| `stats-hidden-ending-hint.test.ts`     | 29     | hiddenEndingHint 进度提示(辩经尊者/执剑尊者双路径)/空记录 false/单笔学术江湖路径/主导非隐藏语气 false/占比 0.5 边界>=阈值/自定义阈值/deedsToUnlock 公式与钳制/count 参数独立/与 endingNarrative 满级一致/纯函数/9 笔差 1 预告语义                                                                 |
| `daily-error-paths-deep.test.ts`       | 49     | fromDateStr 全格式鲁棒(月日溢出/闰年规则/世纪闰/非字符串类型)/toDateStr 往返/dateSeed 非法返 0/dailyReflection 非法日期抛错/upcoming·pastReflections 非法 from 返空·days=0 负返空/跨月跨年边界/pickReflectionQuestions 全 8 题材 3 条/weekday 与 getDay 一致/确定性/allPublicScripts 脱敏（深度） |

**覆盖层级**：shared（纯逻辑，全覆盖）+ server（路由集成，黑盒）。**未覆盖**：前端 src 的 DOM 层（无自动化测试，靠手测）。

**质量门禁**：`npm run type-check`（tsc --noEmit，零错误）+ `npm run lint`（eslint，零错误）+ `npm test`（1598 绿）三件套全过才可提交。
...
npm test             # 1598 个测试（node --test + tsx）
...
- 前端 src 的 DOM 层无自动化测试，shared + server 有测试（1598 用例）——**改 shared 时务必跑 `npm test` + `type-check` + `lint`**。
...
- **R10 轮新增双隐藏结局 + 进度提示**：`ledgerCore.endingNarrative` 扩展为**双隐藏结局**——除既有「辩经尊者」（满级 + 主导学术，用脑解构善恶）外，新增「执剑尊者」（满级 + 主导江湖，用刀贯彻善恶），两者对称、互斥，平局按 `tonePreference` 声明顺序（学术在前）决胜；`EndingNarrative.type` 联合扩展为 `EndingType | 辩经尊者 | 执剑尊者`。`stats.hiddenEndingHint` 是配套的**进度提示引擎**：在玩家尚未满级但已在隐藏路径上时（主导学术/江湖 + 占比 ≥ 阈值默认 0.5）给预告（onPath/path/deedsToUnlock/dominantRatio/unlocked），让前端能展示「你正走向辩经尊者/执剑尊者」。R10 同时把 i18n/export 的内部字典（STRINGS/TONE_LABELS/CATEGORY_LABELS/DIFFICULTY_LABELS/ENDING_NAME）改为导出常量，便于完整性校验。
...
- [ ] **新功能接线**：deep 轮新增的 `dominantTone`（迁移 src/ledger.ts 内联实现）、`endingNarrative`（双隐藏结局展示：辩经尊者/执剑尊者）、`practiceStage`（修行阶段进度条）需接入 src/ 与 cli/。R10 新增的 `stats.hiddenEndingHint`（隐藏结局进度提示）可接入前端「结局预告」横幅——玩家主导学术/江湖且占比 ≥ 50% 时提示「你正走向辩经尊者/执剑尊者，还差 N 笔」。
